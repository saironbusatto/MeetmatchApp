    ...
    <main style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#FAFAF7" }}>
      <div style={{ width: "100%", maxWidth: 400, padding: "0 24px" }}>
        <div style={{
          ...
          padding: "4px 10px 4px 4px", background: "#FFF4BD",
          border: "1.5px solid #0A0A0A", borderRadius: 999,
          boxShadow: "2px 2px 0 #0A0A0A", marginBottom: 18,
        }}>
          <span style={{ background: "#FFD93D", padding: "2px 8px", borderRadius: 999, fontWeight: 700, fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase" as const }}>Grátis</span>
          <span style={{ fontSize: 13, fontWeight: 500, color: "#28272A" }}>Sem cartão, sem frescura</span>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <OAuthButton onClick={() => handleOAuth("oauth_google")} label="Criar com Google" icon={<GoogleIcon />} bg="#fff" fg="#0A0A0A" />
          <OAuthButton onClick={() => handleOAuth("oauth_apple")}  label="Criar com Apple"  icon={<AppleIcon />}  bg="#0A0A0A" fg="#FAFAF7" />
        </div>
